...
| 이름 | 필수 | 기본값 | 설명 |
|---|---|---|---|
| `service-name` | ✅ | — | 서비스 이름 (이미지 태그에 사용) |
...
jobs:
  test:
    ...
    with:
      service-name: your-service   # 1. 서비스 이름
      language: node               # 2. 사용 언어
      node-version: "20.x"        # 3. 해당 언어의 버전 input만 남기기
    secrets:
  ...
  build:
    ...
    with:
      service-name: your-service         # 1. 서비스 이름
      docker-registry: ghcr.io/404-Factory  # 4. 레지스트리 경로
    secrets:
...
| Secret | 등록 위치 | 설명 |
|---|---|---|
| `SONAR_TOKEN` | 각 서비스 repo | SonarCloud 분석 토큰 (없으면 스캔 스킵) |
